    ...
    if (!key) {
        ...
        return ref.once('value').then((emailLinkKey) => {
            ...
            admin.auth().updateUser(uid, {
                ...
                ref = admin.database().ref(`login/${uid}`)
                ref.update({emailLinkKey: ''}) //apaga key
            ...
            res.status(401).send("Ops! Houve um erro na validação do seu e-mail. Por favor entre em contato conosco através do e-mail contato@maisfuturo.com.br")
            return false
        })
    }
})